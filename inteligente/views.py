...
def feeders_list(request):
    ...
    feeders = feeders.order_by("id")

    paginator = Paginator(feeders, 3)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not hasattr(feeder, "next_feeding_time") or not feeder.next_feeding_time:
                ...
                feeder.next_feeding_time = timezone.now() + timedelta(
                    hours=8
            ...
            messages.error(
                request,
                "Erro ao adicionar o alimentador. Verifique os dados e tente novamente.",
            )
    else:
        # Use formulário específico baseado no perfil do usuário
        if is_farmer:
...
def feeder_edit(request, feeder_id):
    ...
    if is_farmer:
        ...
        if feeder.owner != user_full_name:
            messages.error(
                request, "Você não tem permissão para editar este alimentador."
            )
            return redirect("feeders")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                "Erro ao atualizar o alimentador. Verifique os dados e tente novamente.",
            )
    else:
        # Use formulário específico baseado no perfil do usuário
        if is_farmer:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                profile, created = UserProfile.objects.get_or_create(
                    user=user,
                    defaults={"role": role, "phone": phone, "address": address},
                )
            ...
            messages.success(
                request, f'Usuário "{user.get_full_name()}" atualizado com sucesso!'
            )
            return redirect("users")
...
def farmer_profile_edit(request):
    ...
    if not user_profile or user_profile.role != "farmer":
        messages.error(
            request, "Acesso negado. Esta funcionalidade é apenas para agricultores."
        )
        return redirect("dashboard")
    ...
    if request.method == "POST":
        ...
        user_form.fields["password"].required = False
        user_form.fields["confirm_password"].required = False
        user_form.fields["password"].widget.attrs[
            "placeholder"
        ] = "Deixe em branco para não alterar"
        user_form.fields["confirm_password"].widget.attrs[
            "placeholder"
        ] = "Deixe em branco para não alterar"
...
def alerts_list(request):
    ...
    if user_profile and user_profile.role == "farmer":
        ...
        user_full_name = request.user.get_full_name() or request.user.username
        alerts = Alert.objects.filter(feeder__owner=user_full_name).order_by(
            "-created_at"
        )
    else:
        ...
        if form.is_valid():
            ...
            if is_farmer:
                ...
                if alert.feeder.owner != user_full_name:
                    messages.error(
                        request,
                        "Você não tem permissão para criar alertas para este alimentador.",
                    )
                    return redirect("alerts")
            ...
            messages.error(
                request,
                "Erro ao adicionar o alerta. Verifique os dados e tente novamente.",
            )
    else:
        # Use formulário específico baseado no perfil do usuário
        if is_farmer:
